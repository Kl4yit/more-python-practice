...
def reload_game(engine, hero):
    global level_list
    engine.level_list_max = len(level_list) - 1
    engine.level += 1
    hero.position = [1, 1]
    hero.hp = hero.max_hp
    engine.objects = []
    generator = level_list[min(engine.level, engine.level_list_max)]
...
def open_next_dungeon(engine):
    coord = (random.randint(1, 39), random.randint(1, 39))
    intersect = True
    while intersect:
        intersect = False
        if engine.map[coord[1]][coord[0]] == wall:
            intersect = True
            coord = (random.randint(1, 39),
                     random.randint(1, 39))
    engine.objects.append(Objects.Ally(object_list_prob['objects']['stairs']['sprite'],
                                       object_list_prob['objects']['stairs']['action'],
                                       coord))
...
def game_over(engine):
    engine.game_over = True
    reload_game(engine, engine.hero)
...
class SpecialMap(MapFactory):
                ...
                for j in range(41):
                    if i == 0 or j == 0 or i == 40 or j == 40:
                        ...
                        self.Map[j][i] = [wall, floor1, floor2, floor3, floor1,
                                          floor2, floor3, floor1, floor2][1 if j == 1 and i == 2 else random.randint(0, 8)]
    ...
    class Objects:
        ...
        def non_intersect_coords(self, _map, width, height):
            coord = (random.randint(width[0], width[1]), random.randint(height[0], height[1]))
            intersect = True
            while intersect:
                intersect = False
                if _map[coord[1]][coord[0]] == wall:
                    intersect = True
                    coord = (random.randint(width[0], width[1]),
        ...
        def get_objects(self, _map):
            ...
            for obj_name in self.config:
                ...
                for i in range(self.config[obj_name]):
                    coord = self.non_intersect_coords(_map, (1, 39), (1, 39))
...
class EmptyMap(MapFactory):
    ...
    class Objects:
        ...
        def get_objects(self, _map):
            self.objects.append(Objects.Ally(object_list_prob['objects']['stairs']['sprite'], object_list_prob['objects']['stairs']['action'], (37, 5)))
            return self.objects
...
def service_init(sprite_size, full=True):
    ...
    for obj in object_list_prob['objects']:
        ...
        prop['name'] = obj
        object_list_prob['objects'][obj] = prop
    ...
    for ally in object_list_prob['ally']:
        ...
        prop['action'] = object_list_actions[prop_tmp['action']]
        prop['name'] = ally
        object_list_prob['ally'][ally] = prop
    ...
    for enemy in object_list_prob['enemies']:
        ...
        prop['name'] = enemy
        object_list_prob['enemies'][enemy] = prop
